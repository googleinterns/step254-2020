...
let setAccessHtml = '<p id="accessDenied"></p>';
let setAuthHtml = '<p id="logInOut"></p>';
...
test ('check page access false', () => {
  ...
  pageAccess();
  expect( document.getElementById('accessDenied',).innerHTML).toBe(`<p> Cannot access until you login</p>`);
});
...
test ('check prefereneces can be set', async() => {
  global.fetch = jest.fn().mockImplementation(() => Promise.resolve({json: () => mockPreference}));
...
test ('check authentication for valid user', async() => {
  ...
  global.fetch = jest.fn().mockImplementation(() => Promise.resolve({json: () => validUser}));
  authenticate();
  expect(global.fetch).toHaveBeenCalledWith('/auth');
...
test ('check authentication for invalid user', async() => {
  document.body.innerHTML= setAuthHtml;
  global.fetch = jest.fn().mockImplementation(() => Promise.resolve({json: () => invalidUser}));
  authenticate();
  expect(global.fetch).toHaveBeenCalledWith('/auth');
